...
class TelegramHistoryManager:
    ...
    def log_interaction(self, direction: str, chat_id: int, message_type: str, content: dict, update_id: Optional[int] = None):
        ...
        with sqlite3.connect(self.__db_path) as conn:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (datetime.now(), direction, chat_id, update_id, message_type, json.dumps(content)))
            conn.commit()
    ...
    def log_prompt(self, prompt: CurrentPrompt, chat_id: int):
        ...
        with sqlite3.connect(self.__db_path) as conn:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (datetime.now(), 'system', chat_id, 'prompt_start', json.dumps(content), True, 'active'))
            conn.commit()
