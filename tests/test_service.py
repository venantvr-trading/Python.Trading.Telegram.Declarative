...
class TestTelegramService(unittest.TestCase):
    ...
    def test_send_message(self):
        ...
        # Assert
        self.service._TelegramService__sender.send_message.assert_called_once_with(
            message
        )

    def test_flush_outgoing_queue(self):
        """Test outgoing queue flushing."""
        # Act
    ...
    def test_test_updates(self):
        """Test the test_updates method."""
        # Arrange
        expected_updates = {"ok": True, "result": []}
        self.service._TelegramService__client.get_updates.return_value = (
            expected_updates
        )
        ...
        self.service._TelegramService__client.get_updates.assert_called_once_with(
            {"timeout": 5}
        )

    def test_test_updates_with_error(self):
        """Test test_updates with error."""
        # Arrange
        self.service._TelegramService__client.get_updates.side_effect = Exception(
            "Network error"
        )
    ...
    def test_parse_update(self):
        """Test the parse_update method."""
        # Arrange
        update = {"message": {"text": "Hello"}}
        expected_result = (123, "text", {"text": "Hello"})
        self.service._TelegramService__receiver.parse_update.return_value = (
            expected_result
        )
        ...
        self.service._TelegramService__receiver.parse_update.assert_called_once_with(
            update
        )

    def test_parse_command_with_action(self):
        """Test parsing a command with action."""
        # Arrange
        command_update = {"callback_query": {"data": "ask:/test_command:param1;param2"}}
    ...
    def test_parse_command_without_action(self):
        """Test parsing a command without action."""
        # Arrange
        command_update = {"callback_query": {"data": "/simple_command"}}
    ...
    def test_parse_command_invalid_format(self):
        ...
        # Arrange
        command_update = {"callback_query": {"data": "invalid_format"}}
    ...
    def test_cast_to_enum_multiple_enums(self):
        """Test cast with multiple possible enums."""
        # Arrange
        value = "/command"
        mock_enum1 = Mock()
    ...
    def test_start_command_processor(self, mock_thread_class):
        """Test command processor startup."""
        # Arrange
        mock_thread = Mock()
    ...
    def test_start_command_processor_already_running(self):
        """Test startup when processor is already active."""
        # Arrange
        mock_thread = Mock()
        mock_thread.is_alive.return_value = True
        self.service._TelegramService__processor_thread = mock_thread
